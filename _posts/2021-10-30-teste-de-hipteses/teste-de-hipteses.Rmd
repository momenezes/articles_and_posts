...
Então, com a amostra em mãos, efetuamos os cálculos desejados e fazemos a *inferência*, isto é, uma afirmação sobre um *parâmetro* da população. Nossa afirmação pode estar correta (ser verdadeira) ou errada (ser falsa). É comum querermos atribuir um grau de confiância a esta afirmação; algo como "... é possível afirmar, com 95% de confiança, que ..."
...
A distribuição normal é aquela com forma de *sino*; se adotarmos no nosso exemplo que a média de peso da população seja 3.4kg, com um desvio padrão 0.7kg, e a população que foi pesada seja de 800 animais, podemos ter uma *ideia* da distribuição real de probabilidades dos pesos como a mostrada na figura abaixo. Como nossa população é finita, a curva mostrada é uma estimativa da curva de densidade de probabilidade e o formato da curva não é perfeitamente um *sino*, mas se aproxima. O histograma da distribuição de pesos também é mostrado nessa figura.

```{r}
set.seed(1234)
media = 3.4
desvio = 0.7
y <- rnorm(800, mean = media, sd = desvio)
x <- seq(1:800)
...
O eixo *y* representa a *estimativa da densidade de probabilidade de ocorrência de x*. Você pode ler mais sobre Estimativa de Densidade de Probabilidade [aqui](https://bookdown.org/egarpor/PM-UC3M/npreg-npdens.html).
...
g + geom_point(aes(x = `Peso(kg)`, y = P0), data = amostra, color = "blue") +
  ...
  annotate("text", x = 1.05*pesomedioamostra, y = 0.07, label = paste("Méd.Am. ",  round(pesomedioamostra,2)))
```

```{r}
set.seed(1234)

geragrafs <- function(g) {
  amostra <- slice_sample(df, n = 30, replace = TRUE)
  amostra$P0 <- 0
  pesomedioamostra = mean(amostra$`Peso(kg)`)
  novog <- g + geom_point(aes(x = `Peso(kg)`, y = P0), data = amostra, color = "blue") +
               geom_segment(aes(x = pesomedioamostra, y = 0, xend=pesomedioamostra, yend = 0.05), 
                            color = "red") +
               annotate("text", x = 1.05*pesomedioamostra, y = 0.07, 
                        label = paste("Méd.Am.: ",  round(pesomedioamostra,3)))
}

```

Agora com 10 amostras.
...
```{r}
ggpubr::ggarrange(h1,h2,h3,h4,h5,h6,h7,h8,h9,h10, nrow = 5, ncol = 2)
```



Se tomarmos 100 amostras de 30 elementos, e calcularmos a média amostral, teremos o seguinte resultado:

```{r}
set.seed(1234)
mediasamostrais <- function(df, N) {
  medias <- c()
  for (I in seq(1:N)) {
     amostra <- slice_sample(df, n = 30, replace = TRUE)
     amostra$P0 <- 0
...
```{r}
mediasam <- mediasamostrais(df, 1000)
mediasam_est <- t.test(mediasam)
mediasam_mean <- as.numeric(mediasam_est$estimate)
mediasam_confint <- as.numeric(mediasam_est$conf.int)
#plot(density(mediasam))
#hist(mediasam, breaks = 30)

dfmedias <- tibble(x = mediasam)
...
Como observamos na figura acima, a média das médias amostrais (`r round(mediasam_mean, 3)`) tende para a média verdadeira (`r media`) quando tomamos um grande número de amostras.
