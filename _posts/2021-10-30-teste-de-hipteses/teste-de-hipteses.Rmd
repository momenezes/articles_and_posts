...
Mas também temos animais com peso bem maior ou bem menor do que a média; eles são mais raros nas amostras, porque são mais raros também na população.
...
onde $\mu$ é o parâmetro conhecido da população (média) e $\mu_0$ é o valor de teste, obtido da amostra. Quando a hipótese alternativa ($\textrm{H}_1$) é expressa como **diferença** ($\neq$), temos um teste bilateral; quando $\textrm{H}_1$ é expressa como $>$ ou $<$ temos um teste unilateral. 
...
2. Calcular o valor crítico para o nível de significância definido, isto é, quais os limites da variável reduzida que ainda consideramos como *iguais* no teste de hipótese;
...
* media populacional $\mu$: `r media`
* desvio padrão da população: `r desvio`
* tamanho da amostra: `r n`
* média da amostra $\bar{x}$: `r xbar`
* estatística **z**: `r z`
